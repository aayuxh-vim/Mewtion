...
3. **Overlay (Linux)**
   A Rust/GTK4 application renders an always-on-top, click-through canvas. It features a custom 60 FPS particle physics engine with detaching anchors and organic flow patterns.

> **Note on Window Managers:**
> The overlay uses raw X11 (via XWayland) override-redirect windows and shape rectangles. It is confirmed working on KDE/KWin and most other standard compositors.

## Prerequisites

* Linux desktop environment with **X11 or XWayland**
* **Rust / Cargo**
...
Start the overlay using the X11 backend:

```bash
GDK_BACKEND=x11 ./target/release/mewtion
```
...
* [ ] **Windows Support:** Port the X11/GTK window management logic to the Windows API.
* [ ] **Wayland Native Support:** Add native Wayland support where compositor APIs allow it.
...
Make sure you are running under X11 or XWayland:

```bash
GDK_BACKEND=x11 ./target/release/mewtion
```

Mewtion currently relies on X11-compatible overlay functionality.
